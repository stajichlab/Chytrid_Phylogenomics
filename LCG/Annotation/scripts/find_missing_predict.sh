...
do
 ...
 elif [ ! -s  $ANNOTDIR/$name/predict_results/${name}.tbl ]; then
   echo "need to run predict on $name ($N)"
 fi
 N=$(expr $N + 1)
done
...
name=$(echo -n "${SPECIES}_${STRAIN}" | perl -p -e 's/\s+/_/g')
 if [[ -f $INDIR/${name}.sorted.fasta  && -f $OUTDIR/${name}.masked.fasta ]]; then
    if [[ ! -s  $ANNOTDIR/$name/predict_results/${name}.tbl ]]; then
         echo -n "$N,"
    fi
 fi
 N=$(expr $N + 1)
...
echo "sbatch --array=$m pipeline/02_predict.sh"
